    ...
    if request.method == "POST":
        try:
            ...
            for expenditure in expenditures_of_category:
                ...
                expenditure.save()
            Activity.objects.create(user=request.user, image = "images/recover.png",
                                    name = f'You\'ve recovered \"{category.name}\" category with all its expenditures from the bin')
...
def delete_category(request):
    if request.method == "POST":
        try:
            category_pk = request.POST['radio_pk']
            category = Category.objects.get(pk=category_pk)
            Activity.objects.create(user=request.user, image="images/delete.png",
                                    name=f'You\'ve deleted \"{category.name}\" category with all its expenditures')
            category.delete()
