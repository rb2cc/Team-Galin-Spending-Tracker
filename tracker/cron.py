...
def category_progress_notification_cron_job():
    ...
    def _make_percent(num, cat_name, user):
                denom = Category.objects.filter(usersid = user.id).get(name=cat_name).week_limit
    ...
    for user in User.objects.filter(is_staff=False, is_superuser=False):
        ...
        # categories = Category.objects.filter(is_overall = False, usersid=user.id)
        ...
        categories = Category.objects.filter(is_overall = False).filter(usersid = user.id)
        val_dict = {}
        for category in categories:
            val_dict[category.name] = 0 
        expenditures = Expenditure.objects.filter(user=user, date_createdgte = week_start, date_createdlte = week_end, is_binned = False)
        for expenditure in expenditures:
        ...
        overall = Category.objects.filter(usersid = user.id, is_overall=True)
